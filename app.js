...
let entities = [];
let currentTool = 'pan'; // Iniciamos en PUNTERO
let isDrawing = false;
let startWorldPos = null;
let currentEndPos = { x: 0, y: 0 };
let pipeCounter = 1; // Contador para #1, #2, etc.
...
function snapToGrid(worldPos) {
  return {
    x: Math.round(worldPos.x / snapGridMM) * snapGridMM,
    y: Math.round(worldPos.y / snapGridMM) * snapGridMM
  };
}
...
function exportPNG() {
  ...
  const exportZoom = 2; const exportOffsetX = 200; const exportOffsetY = 200;
  entities.forEach(ent => {
    tCtx.strokeStyle = '#000000'; tCtx.lineWidth = ent.diameter_in * 2; tCtx.lineCap = 'round';
    tCtx.beginPath();
  ...
  link.download = 'Achero_Spool.png'; link.href = tempCanvas.toDataURL(); link.click();
}
